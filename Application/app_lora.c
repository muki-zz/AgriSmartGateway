...
EnvironmentData_t g_sensor_data;
rt_sem_t g_mqtt_sem;
 ...
 */
static void set_lora_dio1_callback(){
    rt_sem_release(lora_dio1_sem);
}

/**
 * @brief:校验地址和帧长度，然后按命令处理从机上报的数据
 * @details:有效载荷：2字节co2 + 4字节温度 + 4字节湿度 + 2字节lux + 2字节土壤湿度
*/
static void master_radio_rx(const uint8_t* data, const uint8_t length)
{
    ...
    if (command == CMD_SENSOR_DATA)
    {
        ...
        memcpy(&g_sensor_data.temperature, &temp_raw, sizeof(float));
        p +=4;

        uint32_t humi_raw = ((uint32_t)p[0]<<24) | ((uint32_t)p[1]<<16) | ((uint32_t)p[2]<<8) | p[3];
        memcpy(&g_sensor_data.humidity, &humi_raw, sizeof(float));
        p +=4;
        ...
        g_sensor_data.soil_humidity = ((uint16_t)p[0] <<8) | p[1];
        p +=2;
...
static void lora_init(){
    ...
    gpio_set_exit_callback(LORA_DIO1_Pin,set_lora_dio1_callback);
    lora_dio1_sem=rt_sem_create("dio1",0,RT_IPC_FLAG_PRIO);
 ...
 * @brief:lora线程例程函数
 */
static void lora_thread_entry(void *param){
    llcc68_app_init();
    ...
    while(1){
        rt_err_t sem_ret = rt_sem_take(lora_dio1_sem, rt_tick_from_millisecond(IRQ_POLL_MS));
        if(sem_ret == RT_EOK)
        {
